...
url: markdown/net/mcp/macos-installation
title: Install on macOS
weight: 4
description: "Install the GroupDocs.Markdown MCP server on macOS (Intel and Apple Silicon) via Homebrew-based bootstrap and the guided installer — registered into Claude Desktop and other clients automatically."
...
Install the GroupDocs.Markdown MCP server on **macOS** — Intel and Apple Silicon — via the Homebrew-based bootstrap and the guided installer, registered into Claude Desktop and other clients automatically:
...
pwsh ./install-groupdocs-mcp.ps1 -Channel docker \
  -Products markdown -Clients claude-desktop,cursor -Verify
```
...
Client config locations on macOS: Claude Desktop `~/Library/Application Support/Claude/claude_desktop_config.json`, VS Code user-level `~/Library/Application Support/Code/User/mcp.json`, Cursor `~/.cursor/mcp.json` — full list in [Register in AI clients]({{< ref "markdown/net/mcp/install-in-ai-clients.md" >}}).
...
Installing on a different OS? [Windows]({{< ref "markdown/net/mcp/windows-installation.md" >}}) · [Linux]({{< ref "markdown/net/mcp/linux-installation.md" >}})
