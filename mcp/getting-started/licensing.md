---
id: mcp-licensing
url: markdown/mcp/getting-started/licensing
title: Licensing
weight: 7
description: "The GroupDocs.Markdown MCP server runs in evaluation mode out of the box; unlock it with your existing GroupDocs.Markdown license file or with metered (pay-per-use) keys, and confirm the active mode with the get_license_status tool."
...
The GroupDocs.Markdown MCP server runs in **evaluation mode out of the box** — no sign-up, no key. Your existing GroupDocs.Markdown license unlocks full functionality: point the installer's `licensePath` — or the `GROUPDOCS_LICENSE_PATH` environment variable in a manual install — at your `.lic` file.

**The MCP server itself is open source (MIT); the underlying GroupDocs.Markdown engine requires a license for production use.**
...
| **Evaluation** | nothing | Only the first 3 pages are processed, with an evaluation watermark on each |
...
* Only the **first three pages** are processed — a 200-page handbook converts to three pages of Markdown, and the output does not say the rest was skipped.
* An **evaluation watermark** is placed at the top of each page.

For a RAG ingestion pipeline that is the worst possible failure mode: the index builds successfully and contains almost nothing.
...
```json
{ "licensePath": "D:/Storage/Licenses/GroupDocs.Markdown.lic" }
```

**Manual (NuGet channel)** — set the environment variable in your client's server entry:

```json
"env": { "GROUPDOCS_LICENSE_PATH": "/path/to/GroupDocs.Markdown.lic" }
```
...
{{< alert style="warning" >}}
**Both keys are required.** With only one set, the server ignores the metered configuration and stays in evaluation mode — it reports this explicitly rather than failing silently. Check with [`get_license_status`]({{< ref "markdown/mcp/tools-reference/get-license-status.md" >}}).
{{< /alert >}}
...
Ask your agent *"what is the license status of the Markdown conversion server?"*. The [`get_license_status`]({{< ref "markdown/mcp/tools-reference/get-license-status.md" >}}) tool (server **26.9.0+**) answers without processing a document:
...
If a key pair is rejected, the mode reverts to `evaluation` and the `note` field says why — for example *"Metered keys were supplied but the engine rejected them (Authentication failed.)"* — so a mistyped key surfaces here instead of as three pages of Markdown where you expected two hundred.

## Getting a license

* [Free 30-day temporary license](https://purchase.groupdocs.com/temporary-license/) — full functionality for evaluation.
* [Purchase GroupDocs.Markdown](https://purchase.groupdocs.com/pricing/markdown/net) · [Product page](https://products.groupdocs.com/markdown/net/)
